...
<template>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold gradient-brand-text">{{ isEditing ? 'Edytuj status' : 'Nowy status' }}</h1>
                <p class="text-foreground-muted text-sm mt-1">{{ isEditing ? 'Zaktualizuj dane statusu' : 'Dodaj nowy status zadań' }}</p>
            </div>
            ...
            <Card title="Dane statusu">
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-foreground mb-1">Identyfikator (slug) *</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-foreground mb-1">Kolor *</label>
                            <div class="flex gap-2">
                                <input type="color" v-model="form.color" class="h-10 w-14 rounded border border-border-bright cursor-pointer" />
                                <Input v-model="form.color" class="flex-1" />
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-foreground mb-1">Kolejność</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-2">Szybki wybór koloru</label>
                        <div class="flex flex-wrap gap-2">
                            <button
                                ...
                                :class="form.color === hex ? 'border-foreground ring-2 ring-offset-2 ring-foreground-muted' : 'border-transparent'"
                    ...
                    <!-- Podgląd -->
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-2">Podgląd</label>
            ...
            <Card title="Opcje">
                <div class="space-y-4">
                    <label class="flex items-center">
                        <input type="checkbox" v-model="form.is_default" class="rounded border-border-bright text-brand-primary focus:ring-brand-primary" />
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" v-model="form.is_visible" class="rounded border-border-bright text-brand-primary focus:ring-brand-primary" />
                        <span class="ml-2 text-sm text-foreground">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" v-model="form.is_final" class="rounded border-border-bright text-brand-primary focus:ring-brand-primary" />
                        <span class="ml-2 text-sm text-foreground">
