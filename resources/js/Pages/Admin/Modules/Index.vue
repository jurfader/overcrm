...
<template>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold gradient-brand-text">Zarządzanie modułami</h1>
                <p class="text-foreground-muted text-sm mt-1">Instaluj, konfiguruj i zarządzaj modułami systemu</p>
            </div>
            <div class="flex gap-3">
                <button
                    @click="showGenerateModal = true"
                    class="inline-flex items-center gap-2 px-4 py-2 surface-elevated border border-border-bright rounded-lg hover:bg-surface-elevated/50 font-medium text-foreground"
                >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="p-6">
                    <div class="flex items-start justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="font-semibold text-foreground">{{ module.display_name }}</h3>
                                <p class="text-xs text-foreground-muted">v{{ module.version }}</p>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            :class="[
                                ...
                                module.is_active ? 'bg-brand-primary' : 'bg-surface-elevated',
                    ...
                    <p class="mt-3 text-sm text-foreground-muted line-clamp-2">
                    ...
                    <div class="mt-4 flex items-center gap-2">
                        ...
                        <span v-else class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium surface-elevated text-foreground-muted">
                            Nieaktywny
                        </span>
                    </div>
                </div>

                <div class="px-6 py-3 bg-surface-2/50 border-t border-border flex items-center justify-between">
                    <span class="text-xs text-foreground-muted">{{ module.author || 'Nieznany autor' }}</span>
                    <div class="flex gap-2">
                        <Link
                            v-if="module.has_settings"
                            :href="route('admin.modules.show', module.id)"
                            class="text-brand-primary hover:opacity-80 text-sm font-medium"
                        >
                            Konfiguracja
                        </Link>
                    </div>
            ...
            <div v-if="modules.length === 0" class="col-span-full text-center py-12 text-foreground-muted">
    ...
    <div v-if="showInstallModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="glass-card shadow-2xl w-full max-w-md mx-4">
            <div class="flex items-center justify-between px-6 py-4 border-b border-border">
                <h3 class="text-lg font-semibold text-foreground">Zainstaluj moduł z pliku</h3>
                <button @click="showInstallModal = false" class="p-1 rounded-lg hover:bg-surface-elevated">
            ...
            <form @submit.prevent="uploadModule" class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-foreground mb-2">Plik modułu (.zip)</label>
                    <input
                        type="file"
                        @change="e => installForm.module_file = e.target.files[0]"
                        accept=".zip"
                        class="block w-full text-sm text-foreground-muted file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100"
                    />
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <button type="button" @click="showInstallModal = false" class="px-4 py-2 border border-border-bright rounded-lg hover:bg-surface-elevated/50 text-foreground">
                        Anuluj
                    </button>
    ...
    <div v-if="showGenerateModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="glass-card shadow-2xl w-full max-w-lg mx-4">
            <div class="flex items-center justify-between px-6 py-4 border-b border-border">
                <h3 class="text-lg font-semibold text-foreground">Utwórz nowy moduł</h3>
                <button @click="showGenerateModal = false" class="p-1 rounded-lg hover:bg-surface-elevated">
                    <Icons name="close" class="w-5 h-5 text-foreground-muted" />
                </button>
            </div>
            ...
            <form @submit.prevent="createModule" class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-foreground mb-1">Nazwa techniczna *</label>
                    <input
                        v-model="generateForm.name"
                        type="text"
                        placeholder="np. crm, invoices, reports"
                        class="w-full px-3 py-2 border border-border-bright bg-surface-elevated rounded-lg focus:ring-2 focus:ring-brand-primary"
                        required
                    />
                    <p class="text-xs text-foreground-muted mt-1">Tylko małe litery, bez spacji</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-foreground mb-1">Nazwa wyświetlana *</label>
                    <input
                ...
                <div>
                    <label class="block text-sm font-medium text-foreground mb-2">Ikona</label>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <button type="button" @click="showGenerateModal = false" class="px-4 py-2 border border-border-bright rounded-lg hover:bg-surface-elevated/50 text-foreground">
                        Anuluj
                    </button>
